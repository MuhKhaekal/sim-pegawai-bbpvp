...
export default function BidangUnitKerjaClient({ initialData }: Props) {
  ...
  const [selected, setSelected] = useState<BidangUnitKerja | null>(null);
  ...
  function openEdit(item: BidangUnitKerja) {
    setSelected(item);
    setError("");
    setModal("edit");
  }

  function openDelete(item: BidangUnitKerja) {
    setSelected(item);
    setError("");
    setModal("hapus");
  }
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (modal === "tambah") {
        const result = await tambahBidangUnitKerja(formData);
        if (!result.success) throw new Error(result.message);
      } else if (modal === "edit") {
        const result = await updateBidangUnitKerja(formData);
        if (!result.success) throw new Error(result.message);
      } else if (modal === "hapus" && selected) {
        // Berdasarkan original code, hapusBidangUnitKerja menerima parameter id (number)
        const result = await hapusBidangUnitKerja(selected.id);
        if (!result.success) throw new Error(result.message);
      }
    ...
    <div className="p-6 md:p-10 max-w-7xl mx-auto">
      ...
      <div className="animate-fade-up flex flex-col lg:flex-row lg:justify-between lg:items-end gap-5 mb-6">
        <div>
          ...
          <h1 className="text-3xl lg:text-4xl font-black text-[#15406A] tracking-tight">Bidang & Unit Kerja</h1>
          <p className="text-gray-500 mt-1 lg:mt-2 font-medium text-sm lg:text-base">
            Kelola daftar Bidang/Unit Kerja yang digunakan oleh data pegawai institusi.
          </p>
        </div>
      ...
      <div className="grid gap-4 sm:grid-cols-2 mb-6">
        <StatCard label="Total Bidang / Unit Kerja" value={initialData.length} />
        <StatCard label="Hasil Pencarian" value={filteredData.length} />
      </div>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-gray-50 border-b border-gray-200">
                ...
                <th className="px-4 py-3 text-[11px] font-bold text-gray-500 uppercase tracking-wider">Bidang / Unit Kerja</th>
              ...
              {filteredData.length === 0 ? (
                <tr>
                  <td colSpan={3} className="text-center py-10 text-sm text-gray-500">
                    {search ? "Tidak ada bidang yang sesuai pencarian." : "Belum ada data bidang / unit kerja."}
                  </td>
                </tr>
              ) : (
                filteredData.map((item, index) => (
                  <tr key={item.id} className={`group border-b border-gray-100 hover:bg-blue-50/40 transition-colors ${index % 2 === 0 ? "bg-white" : "bg-gray-50/30"}`}>
                    ...
                    <td className="px-4 py-3 align-middle">
                      <span className="text-sm font-bold text-gray-900 group-hover:text-[#15406A] transition-colors">{item.nama_bidang}</span>
                    </td>
            ...
            {modal === "hapus" ? (
              <form onSubmit={handleSubmit}>
                <div className="p-6 md:p-8">
                  ...
                  <h3 className="text-xl font-bold text-center text-gray-900 mb-2">Hapus Bidang/Unit Kerja?</h3>
                  <p className="text-center text-gray-500 mb-6 text-sm">
                    Anda yakin ingin menghapus <strong>{selected?.nama_bidang}</strong>? Tindakan ini tidak dapat dibatalkan.
                  </p>
              ...
              <form onSubmit={handleSubmit}>
                ...
                <div className="p-6 md:p-8">
                  <div className="mb-6 border-b border-gray-100 pb-4">
                    <h3 className="text-xl font-bold text-[#15406A]">{modal === "edit" ? "Edit Bidang/Unit Kerja" : "Tambah Bidang/Unit Kerja"}</h3>
                    <p className="text-sm text-gray-500 mt-1">Lengkapi informasi master data institusi.</p>
                  </div>

                  <div className="space-y-4 mb-6">
                    <div>
                      <label className="mb-1.5 block text-sm font-bold text-gray-700">Nama Bidang / Unit Kerja</label>
                      <input
                        name="nama_bidang"
                        required
                        autoFocus
                        defaultValue={selected?.nama_bidang ?? ""}

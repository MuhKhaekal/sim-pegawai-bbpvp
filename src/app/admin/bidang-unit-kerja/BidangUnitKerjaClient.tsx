...
import {
  tambahBidangUnitKerja,
  updateBidangUnitKerja,
  hapusBidangUnitKerja,
} from "./actions";
...
export default function BidangUnitKerjaClient({
  initialData,
}: Props) {
  ...
  const [selectedData, setSelectedData] =
    useState<BidangUnitKerja | null>(null);

  const [namaBidang, setNamaBidang] =
    useState("");
  ...
  const handleSubmit = async (
    ...
    try {
      ...
      const result =
        modalMode === "edit"
          ? await updateBidangUnitKerja(formData)
          : await tambahBidangUnitKerja(formData);
  ...
  const handleDelete = async (
    item: BidangUnitKerja,
  ) => {
    const confirmed = window.confirm(
      `Hapus "${item.nama_bidang}"?`,
    );
    ...
    try {
      const result =
        await hapusBidangUnitKerja(item.id);
      ...
      <div className="mx-auto max-w-7xl space-y-6">
        ...
        <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
          <div>
            ...
            <h1 className="text-3xl font-black tracking-tight text-[#15406A] md:text-4xl">
              Bidang / Unit Kerja
            </h1>

            <p className="mt-1 text-sm font-medium text-slate-500">
              Kelola daftar Bidang/Unit Kerja yang
              digunakan oleh data pegawai.
            </p>
          </div>
          ...
          <button
            type="button"
            onClick={bukaTambah}
            className="rounded-xl bg-[#15406A] px-5 py-3 text-sm font-bold text-white shadow-lg transition hover:bg-[#103653] active:scale-[0.98]"
          >
            + Tambah Data
          </button>
        </div>
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
          <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
            <p className="text-xs font-bold uppercase tracking-wider text-slate-400">
              Total Bidang / Unit Kerja
            </p>

            <p className="mt-2 text-3xl font-black text-[#15406A]">
              {initialData.length}
            </p>
          </div>
            ...
            <table className="w-full min-w-[650px] text-sm">
              <thead>
                <tr className="border-b border-slate-200 bg-slate-50 text-left">
                  <th className="px-5 py-4 font-black text-slate-500">
                    No
                  </th>
                  ...
                  <th className="px-5 py-4 text-right font-black text-slate-500">
                    Aksi
                  </th>
                </tr>
              </thead>
                      ...
                      <td className="px-5 py-4">
                        <div className="flex justify-end gap-2">
                          <button
                            ...
                            className="rounded-lg border border-blue-200 bg-blue-50 px-3 py-2 text-xs font-bold text-[#15406A] transition hover:bg-blue-100"
                          >
                            Edit
                          </button>
        ...
        <div className="fixed inset-0 z-[99999] flex items-center justify-center bg-slate-950/50 p-4 backdrop-blur-sm">
          <div className="w-full max-w-lg rounded-2xl bg-white shadow-2xl">
            ...
            <div className="border-b border-slate-200 px-6 py-5">
              <h2 className="text-xl font-black text-[#15406A]">
                {modalMode === "edit"
                  ? "Edit Bidang / Unit Kerja"
                  : "Tambah Bidang / Unit Kerja"}
              </h2>

              <p className="mt-1 text-sm text-slate-500">
                Masukkan nama Bidang/Unit Kerja.
              </p>
            </div>
            ...
            >
              ...
              <div className="flex justify-end gap-3 border-t border-slate-100 pt-5">
                ...
                <button
                  type="submit"
                  disabled={isLoading}
                  className="rounded-xl bg-[#15406A] px-5 py-3 text-sm font-bold text-white transition hover:bg-[#103653] disabled:cursor-not-allowed disabled:opacity-60"
                >
                  {isLoading
                    ? "Menyimpan..."
                    : modalMode === "edit"
